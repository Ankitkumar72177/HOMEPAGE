document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderBookmarks() {
        ...
        for (let i = 0; i < pagesNeeded; i++) {
            ...
            if (pageBookmarks.length < BOOKMARKS_PER_PAGE || (totalBookmarks === 0 && i === 0)) {
        ...
        // Always ensure there's at least one page with an add button
        // If the last page is full, create a new empty page with just the add button
        const lastPageIndex = pagesNeeded - 1;
        const lastPageStart = lastPageIndex * BOOKMARKS_PER_PAGE;
        const lastPageEnd = Math.min(lastPageStart + BOOKMARKS_PER_PAGE, totalBookmarks);
        const lastPageBookmarksCount = lastPageEnd - lastPageStart;
        
        if (lastPageBookmarksCount === BOOKMARKS_PER_PAGE) {
            ...
            // Update pagesNeeded to include this new page
            pagesNeeded += 1;
        }
        ...
        renderPaginationDots(pagesNeeded);
    ...
    function createBookmarkElement(bookmark) {
        ...
        bookmarkCard.innerHTML = `
            ...
            <img src="${bookmark.favicon}" alt="${displayName}" class="bookmark-logo" onerror="this.src='https://via.placeholder.com/64?text=${displayName.charAt(0)}'">
    ...
    function renderPaginationDots(pagesCount) {
        ...
        // Only use pagesCount if it's provided, otherwise use calculated pages
        let totalPages = pagesCount || regularPages;
        
        // Only add an extra page if the last page is completely full AND we have bookmarks
        // This fixes the issue of having an extra dot when there are few bookmarks
        const lastPageIsFull = totalBookmarks > 0 && totalBookmarks % BOOKMARKS_PER_PAGE === 0;
        if (lastPageIsFull && !pagesCount) {
            totalPages++;
        }
        
        // Create dots only if we have more than 0 total pages
        ...
        // Make the pagination dots container visible only if there are multiple pages
        if (totalPages > 1) {
